...
/**
 * Controller for handling admin profile management:
 * viewing, updating, and deleting profile information.
 */
...
public class AdminProfileController extends HttpServlet {
    ...
    /**
     * Initializes the servlet by setting up DAO and utility classes.
     */
    @Override
    ...
    /**
     * Handles GET requests to fetch and display the admin's profile information.
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            ...
            if (username != null) {
                ...
                if (user != null) {
                    if (user.getImage_path() != null && !user.getImage_path().isEmpty()) {
                        ...
                        String imageFullPath = applicationPath + "resources" + File.separator + "image" +
                                File.separator + "user" + File.separator + user.getImage_path();
                        File imageFile = new File(imageFullPath);
                        if (!imageFile.exists()) {
                            System.out.println("WARNING: Profile image not found: " + imageFullPath);
        ...
        request.getRequestDispatcher("/WEB-INF/jspfiles/adminprofile.jsp").forward(request, response);
    }

    /**
     * Handles POST requests to update the admin profile.
     * Validates input fields and optionally handles profile image upload.
     * Updates the user in the database and redirects to the profile view.
     */
    @Override
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
         ...
         // Handle image upload
            Part imagePart = request.getPart("myfile");
            String originalFileName = imageUtil.getImageNameFromPart(imagePart);
            String extension = originalFileName.substring(originalFileName.lastIndexOf('.'));
            String imageName = "IMG_" + DateTimeFormatter.ofPattern("yyyyMMddHHmmss").format(LocalDateTime.now()) + extension;
    ...
    /**
     * Handles DELETE requests to delete the admin's account.
     * If successful, the session is invalidated.
     */
    @Override
    ...
    /**
     * Validates profile form data and returns an error message if validation fails.
